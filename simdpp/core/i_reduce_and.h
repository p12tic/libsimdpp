...
int16_t reduce_and(const int8<N,E>& a)
{
    return detail::insn::i_reduce_and(uint8<N>(a.eval()));
}

template<unsigned N, class E> SIMDPP_INL
uint16_t reduce_and(const uint8<N,E>& a)
{
    return detail::insn::i_reduce_and(a.eval());
}

template<unsigned N, class E> SIMDPP_INL
int32_t reduce_and(const int16<N,E>& a)
{
    return detail::insn::i_reduce_and(uint16<N>(a.eval()));
}

template<unsigned N, class E> SIMDPP_INL
uint32_t reduce_and(const uint16<N,E>& a)
{
    return detail::insn::i_reduce_and(a.eval());
}

template<unsigned N, class E> SIMDPP_INL
int64_t reduce_and(const int32<N,E>& a)
{
    return detail::insn::i_reduce_and(uint32<N>(a.eval()));
}

template<unsigned N, class E> SIMDPP_INL
uint64_t reduce_and(const uint32<N,E>& a)
{
    return detail::insn::i_reduce_and(a.eval());
}
