...
#include <simdpp/detail/insn/bitselect.h>
...
template<unsigned N, class V1, class V2, class V3> SIMDPP_INL
    ...
    bitselect(const any_vec<N,V1>& on, const any_vec<N,V2>& off,
              const any_vec<N,V3>& mask)
{
    using E = typename detail::get_expr_blend<V1, V2, V3>;
    return detail::insn::i_bitselect(typename E::v1_final_type(on.wrapped()),
