...
#ifndef LIBSIMDPP_SIMDPP_ALGORITHM_FIND_IF_NOT_H
#define LIBSIMDPP_SIMDPP_ALGORITHM_FIND_IF_NOT_H
...
namespace simdpp {
    namespace SIMDPP_ARCH_NAMESPACE {
        ...
        T const* find_if_not(T const* first, T const* last, UnaryPredicate pred)
        {
            ...
            const auto resprologue = std::find_if_not(first, lastprologue, pred);
            ...
            {
                //TR why can't reduce_and not available for mask?
                const auto res = simdpp::blend(on, off, pred(simdpp::load(lastprologue)));
                if (!simdpp::reduce_and(res)) //match extract exact position
                {
                    return std::find_if_not(lastprologue, lastprologue + simd_size, pred); //or extract position from res ?
                }
                lastprologue += simd_size;
            }

            //epilogue
            return std::find_if_not(lastprologue, last, pred);
        }

    } // namespace SIMDPP_ARCH_NAMESPACE
} // namespace simdpp
#endif //LIBSIMDPP_SIMDPP_ALGORITHM_FIND_IF_NOT_H