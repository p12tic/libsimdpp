...
#ifndef LIBSIMDPP_SIMDPP_ALGORITHM_COUNT_IF_H
#define LIBSIMDPP_SIMDPP_ALGORITHM_COUNT_IF_H
...
count_if(T const* first, T const* last, UnaryPredicate pred)
{
    ...
    if (!first)
        throw std::runtime_error("count_if - null ptr first.");
    if (!last)
        throw std::runtime_error("count_if - null ptr last.");
#endif
    ...
    return_type res = std::count_if(first, lastprologue, pred);
...
#endif //LIBSIMDPP_SIMDPP_ALGORITHM_COUNT_IF_H
