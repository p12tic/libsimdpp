...
public:
    ...
    {
        *this = bit_cast<float32<8>>(d.wrapped());
    }
    template<class V> SIMDPP_INL float32<8>& operator=(const any_vec<32,V>& d)
    {
        *this = bit_cast<float32<8>>(d.wrapped()); return *this;
    }
    ...
    template<unsigned N> const float32<8>& vec() const { return *this; }
    template<unsigned N> float32<8>& vec() { return *this; }
...
public:
    ...
    template<unsigned N> const mask_float32<8>& vec() const { return *this; }
    template<unsigned N> mask_float32<8>& vec() { return *this; }
