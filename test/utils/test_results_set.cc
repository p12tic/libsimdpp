                                             ...
                                             const char* file, unsigned line)
{
    ...
    curr_part.push_back(Result(type, length, element_size_for_type(type), file, line, seq_++,
...
void print_arch(std::ostream& err, const char* a_arch, const char* b_arch)
{
    err << "  For architectures: " << a_arch << " and " << b_arch << " :\n";
}

void fmt_test_case(std::ostream& err, const TestResultsSet& a)
{
                            ...
                            TestReporter& tr)
{
    ...
    for (unsigned is = 0; is < a.results().size(); is++) {
        const std::vector<TestResultsSet::Result>& sect_a = a.results()[is];
        const std::vector<TestResultsSet::Result>& sect_b = b.results()[is];
        ...
        for (unsigned i = 0; i < sect_a.size(); ++i) {
            const TestResultsSet::Result& ia = sect_a[i];
            const TestResultsSet::Result& ib = sect_b[i];
