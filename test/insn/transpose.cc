...
void test_transpose_n(TestResultsSet& tc)
{
    ...
    TEST_PUSH_ARRAY_SPAN(tc, uint8<B>, v.u8, v.size());
    v.reset();

    // int16
    transpose4(v.u16[0], v.u16[1], v.u16[2], v.u16[3]);
    TEST_PUSH_ARRAY_SPAN(tc, uint16<B/2>, v.u16, v.size());
    v.reset();
    ...
    TEST_PUSH_ARRAY_SPAN(tc, uint32<B/4>, v.u32, v.size());
    v.reset();
    ...
    TEST_PUSH_ARRAY_SPAN(tc, uint64<B/8>, v.u64, v.size());
    v.reset();

    // float32
    transpose4(v.f32[0], v.f32[1], v.f32[2], v.f32[3]);
    TEST_PUSH_ARRAY_SPAN(tc, float32<B/4>, v.f32, v.size());
    v.reset();
    ...
    TEST_PUSH_ARRAY_SPAN(tc, float64<B/8>, v.f64, v.size());
    v.reset();
}
