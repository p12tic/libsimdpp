...
template<typename T>
struct CountFuzzingTest
{
    CountFuzzingTest(std::initializer_list<std::size_t> sizes = {}) :m_sizes(sizes), m_generator(42) {}
...
void test_count_type(TestResultsSet& ts, TestReporter& tr)
{
    ...
    CountFuzzingTest<T> fuzzing({ 1,3,5,8,21,55,89,144 });//0 generate null ptr inputs/ouput
    fuzzing(tr);
}
